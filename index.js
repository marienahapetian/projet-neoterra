...
function create_question_layout(id) {
  ...
  question_nav.textContent = `Question ${user_responses.length + 1}/${
    questions.length
  }`;
  ...
  timer_timeout = globalThis.setTimeout(
...
function create_bilan_container() {
  ...
  bilan_text.textContent = `Quizz terminé ! Bilan final : ${get_total_points()}/${
    user_responses.length
  }`;
...
function save_progress() {
  sessionStorage.setItem(
    "current_question_id",
    JSON.stringify(current_question_id)
  );
...
function load_progress() {
  current_question_id = JSON.parse(
    sessionStorage.getItem("current_question_id")
  );
...
function update_response_timer() {
  ...
  const time_left =
    response_delay - Math.floor((Date.now() - timer_start) / 1000);
...
async function get_questions_data() {
  const json_file = "./questions-sante.json";
